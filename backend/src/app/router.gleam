...
pub fn successful_fn() {
  use <- tracing.with_span("successful_fn", [])
  Nil
}

pub fn error_fn() {
  use <- tracing.with_span("error_fn", [])
  tracing.set_span_error_message("Crazy things happened")
  Error("Crazy things happened")
}

pub fn test_handler(req: Request) -> Response {
  use <- tracing.with_span("test_handler", [])
  successful_fn()
  case req.method {
    Get -> {
      let _ = error_fn()
...
fn list_comments() -> Response {
  use <- tracing.with_span("list_comments", [])
